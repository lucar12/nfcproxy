...
public class TagHelper {
    private static final String TAG = "TAG_PARSER";
    ...
    public static String parseTrack1(byte[] track1, int offset, boolean mask) {
        ...
        //TODO: check masks, if removed, what happen
        sb.append("\nCard Number: ");
        if (!mask)
            ccnum = new String(track1, offset, PANLength);
        else
        ...
        sb.append("\nExpiration Date: ");
        if (!mask) {
            String yy = new String(track1, expOffset, 2);
            String mm = new String(track1, expOffset + 2, 2);
            exp = mm + "/" + yy;
        }
        else
            exp = "XX/XX";
    ...
    public static String parseTrack2(byte[] track2, int offset, boolean mask) {
        String ccnum = "", exp = "", scode = "";
        Formatter format;
        int PANLength = 8;
        int expOffset = offset + PANLength;             // j
        int svcOffset = offset + PANLength + 2;         // k
        ...
        format = new Formatter();
        byte y1 = 0, y0 = 0, m1 = 0, m0 = 0;
        sb.append("\nExpiration Date: ");
        if (!mask) {
            y1 |= track2[expOffset] & 0x0F;
            y0 |= track2[expOffset + 1] & 0xF0;
            y0 = (byte) ((y0 >> 4) & 0x0F);
            m1 |= track2[expOffset + 1] & 0x0F;
            m0 |= track2[expOffset + 2] & 0xF0;
            m0 = (byte) ((m0 >> 4) & 0x0F);
            exp += format.format("%1x%1x/%1x%1x", m1, m0, y1, y0).toString();
        }
        else
            exp = "XX/XX";
        sb.append(exp);
